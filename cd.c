...
/*   Updated: 2023/09/03 21:15:38 by nait-ali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int change_directories(char *pwd, char *old_pwd) {
    char *pwd_;
    char *oldpwd_;
    ...
    if (!search_oldpwd(old_pwd)) {
        oldpwd_ = ft_strjoin("OLDPWD=", old_pwd);
        if (!oldpwd_) {
            perror("ft_strjoin");
            return 1;
        }
        lstadd_back_environement(&an.environement, creation_node_in_env(oldpwd_, ft_strdup("OLDPWD"), ft_strdup(old_pwd)));
        free(oldpwd_);
    }
    return 1;
}
...
void ft_cd(t_cmd *cmd) {
    ...
    char *from;
    char *to;
    t_substruct *var = cmd->s_substruct->next;
